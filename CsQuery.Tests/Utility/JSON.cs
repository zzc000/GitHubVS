...
namespace CsQuery.Tests.Utility
{
    ...
    public class JSON_
    {
        ...
        public void NonStandard()
        {
            ...
            dynamic obj = new JsObjectWrapper(CQ.ParseJSON("{test: 'value',number: 2}") as JsObject);
            
            Assert.AreEqual("value", obj.test, "Could parse JSON with no quotes");

            obj = new JsObjectWrapper(CQ.ParseJSON("{'test':  {'subprop1': 'subproperty1 value', 'subprop2': null }, 'number': 2}") as JsObject);

            Assert.AreEqual("subproperty1 value", ((dynamic)(new JsObjectWrapper(obj.test))).subprop1);
